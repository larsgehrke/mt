...
class KernelTensors:
    ...
    def reset(self):
        ...
        # Inputs
        self.pk_dyn_in = th.zeros(size=(self.config.batch_size,
                                        self.config.amount_pks,
                                        self.config.pk_dyn_in_size),
                                  device=self.config.device)
        self.pk_lat_in = th.zeros(size=(self.config.batch_size,
                                        self.config.amount_pks,
        ...
        # LSTM states
        self.pk_lstm_c = th.zeros(size=(self.config.batch_size, 
                                        self.config.amount_pks, 
                                        self.config.pk_num_lstm_cells),
                                  device=self.config.device,
                                  requires_grad=True)
        self.pk_lstm_h = th.zeros(size=(self.config.batch_size, 
                                        self.config.amount_pks, 
                                        self.config.pk_num_lstm_cells),
                                  device=self.config.device,
                                  requires_grad=True)
        ...
        # Outputs
        self.pk_dyn_out = th.zeros(size=(self.config.batch_size, 
                                         self.config.amount_pks,
                                         self.config.pk_dyn_out_size),
                                   device=self.config.device)
        self.pk_lat_out = th.zeros(size=(self.config.batch_size, 
                                         self.config.amount_pks,
                                         self.config.pk_neighbors,
